...
run_scenario "Gateway API (without Ingress)" "${VALUES_DIR}/gateway-only.yaml"
...
helm template "$RELEASE_NAME" "$CHART_DIR" \
  -f "${VALUES_DIR}/smp-shared-kubernetes-secrets.yaml" \
  -f "${VALUES_DIR}/smp-shared-kubernetes-secrets-customer-override.yaml" >/dev/null
